...
| `/api/health` | Unauthenticated health check (`{ ok, requestId }`) for hosting/uptime probes |
...
- **Group capacity** enforced on assignment (`POST /api/group/:id/student`, unless `allowOverfill`) AND direct student edits (`PUT /api/student/:id` with `groupId`) — full group returns `409 GROUP_FULL` and flips `status` to `FULL`. Lead **conversion deliberately allows overfill** (staff decides placement). Transfers and removals refresh the source group's status (a FULL group is released back to ACTIVE), and removal via `DELETE /api/group/:id/student/:studentId` verifies the student belongs to the addressed group (`409 NOT_IN_GROUP` / `404 STUDENT_NOT_FOUND`).
- **Atomic lead conversion:** `convertLead` runs in a `prisma.$transaction` with a unique constraint on `Student.leadId` as race backstop — a concurrent duplicate convert gets `409 ALREADY_CONVERTED` (direct `POST /api/student` gets `409 LEAD_ALREADY_CONVERTED`); the welcome-sequence enqueue is post-commit. Lead↔Student is 1:1: lead detail returns `student` (object or null), not `students`.
